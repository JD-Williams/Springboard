...
SELECT COUNT(DISTINCT name) AS free_facilities
FROM `Facilities`
WHERE membercost = 0
...
FROM `Facilities` 
WHERE membercost BETWEEN 0.01 AND (0.2 * monthlymaintenance)
ORDER BY membercost DESC
...
SELECT sub.facility_name, sub.total_revenue
FROM (
SELECT F.name AS facility_name,
CASE WHEN M.memid = 0 THEN B.slots * 0.5 * F.guestcost
ELSE B.slots * F.membercost END AS user_revenue,
SUM(2) AS total_revenue
FROM country_club.Bookings B
INNER JOIN country_club.Members M
ON B.memid = M.memid 
INNER JOIN country_club.Facilities F
